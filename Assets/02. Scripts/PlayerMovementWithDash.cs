...
public class PlayerMovement : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!IsDashing && !IsJumping)
		{
			...
			//Right Wall Check - only set if not moving away from wall
			if (((Physics2D.OverlapBox(_frontWallCheckPoint.position, _wallCheckSize, 0, _groundLayer) && IsFacingRight)
					|| (Physics2D.OverlapBox(_backWallCheckPoint.position, _wallCheckSize, 0, _groundLayer) && !IsFacingRight)) && !IsWallJumping && _moveInput.x >= 0)
				LastOnWallRightTime = Data.coyoteTime;

			//Left Wall Check - only set if not moving away from wall
			if (((Physics2D.OverlapBox(_frontWallCheckPoint.position, _wallCheckSize, 0, _groundLayer) && !IsFacingRight)
				|| (Physics2D.OverlapBox(_backWallCheckPoint.position, _wallCheckSize, 0, _groundLayer) && IsFacingRight)) && !IsWallJumping && _moveInput.x <= 0)
		...
		{
			if (!IsSliding) //Just started sliding
			{
    ...
    private void FixedUpdate()
	{
		...
		//Prevent rotation (alternative to Freeze Rotation constraint)
		transform.rotation = Quaternion.identity;
    }

    #region INPUT CALLBACKS
	//Methods which handle input from New Input System
	...
	private void Slide()
	{
		//Calculate how long we've been sliding
		float slideTime = Time.time - _wallSlideStartTime;
		
		//Start slow, then accelerate over time
		float minSlideSpeed = Data.slideSpeed * 0.2f; //Start at 20% of max speed
		float currentSlideSpeed = Mathf.Lerp(minSlideSpeed, Data.slideSpeed, slideTime / 1.5f); //Reach max speed after 1.5 seconds
		
		//Directly set velocity for reliable wall sliding
